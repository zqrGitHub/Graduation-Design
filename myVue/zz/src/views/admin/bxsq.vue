  ...
  <div>
    ...
    <el-card shadow="always">
      <el-table :data="repairsList" stripe border height="310">
        ...
        <el-table-column label="损坏图片" prop="uploadimg">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <!-- 是否已处理开关 -->
            <el-tooltip effect="dark" :content="scope.row.switchon == '1' ? '已处理' : '未处理'" placement="top" :enterable="false">
...
export default {
  ...
  methods: {
    ...
    oneQuest() {
      ...
      this.axios
        .post(
          `http://localhost:3001/getadbxsq`, params
        )
        .then((res) => {
          // // 总条数量
    ...
    userStateChanged(user) {
      ...
      this.axios.post('http://localhost:3001/getAll/onoff', params).then(res => {
    ...
    removeUserById(userid) {
      ...
      this.$confirm("此操作将永久删除报修信息, 是否继续?", "提示", {
        ...
        .then(() => {
          // 发送请求
          this.axios
            .post(`http://localhost:3001/getAllbxsq/delete?id=${id}`)
            .then((res) => {
              // if (res.data.code == 200) {
              console.log(res);
              // }
