...
export default {
  ...
  mounted() {
    ...
    this.axios
      .get(`http://localhost:3001/getUser?username=${username}`)
      .then((res) => {
        console.log(res);
        this.userForm.name = res.data.data[0].sname;
        this.userForm.sex = res.data.data[0].sex ? "男" : "女";
        this.userForm.uid = res.data.data[0].snumber;
        this.userForm.password = res.data.data[0].spassword;
        this.userForm.tel = res.data.data[0].stelphone;
        this.userForm.address = res.data.data[0].saddress;
      });
  },
  methods: {
    // 显示可修改的信息
    ...
    submit() {
      ...
      // 提交之前给个选项是否确定修改信息
      this.$confirm("此操作将修改您的信息, 是否继续?", "提示", {confirmButtonText: "确定",cancelButtonText: "取消", type: "warning",}).then(() => {
          // 用户确认要修改信息
          // 判断他输入的值是否合理，通过了正则验证我再发送请求
      if (this.telCheck() && this.passwordCheck() && this.addressCheck()) {this.axios.put("http://localhost:3001/updateUser", params).then((res) => {if (res.data.code == 200) {
      this.$message({showClose: true,message: "信息修改成功",type: "success",});
                  this.reload();
                } else {
                  this.$message({
                    showClose: true,
